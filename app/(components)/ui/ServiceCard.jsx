...
const ServiceCard = ({services}) => {
  return (
    ...
    //  services.map((service) =>(
      ...
      <div key={services?.id} className=' w-full md:w-[230px] flex flex-col items-start gap-y-4'>
   ...
   <h3 className='font-bold text-xl text-slate-950 underline'>{services?.title}</h3>
   <p className='text-[#788082] '>{services?.text}</p>
      </div>
    // ))
    // }
  )
